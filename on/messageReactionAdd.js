  ...
  return getItem(msgId)
    ...
    .then(() => {
      return getSuspiciousPeers(user.id).then((peerIds) => {
        ...
        //is the item being voted on amongst those that are suspicious
        const currentItemIsSupicious = peerIds.some((suspicion) =>
          suspicion.items.some(
            (item) => item.messageId == messageReaction.message.id
          )
        );
        if (currentItemIsSupicious) return Promise.resolve();
        ...
        logChannel = messageReaction.message.guild.channels.cache.get(
          process.env.LOG
        );
